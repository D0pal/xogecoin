...
#
# Be aware that xogecoind and xogecoin-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in xogecoind.
#
...
# C locale.
#
# In contrast, xogecoind does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where xogecoind is guaranteed to be running
# with the classic locale ("C") whereas the locale of xogecoin-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in xogecoind but
# "1,230000" in xogecoin-qt.
#
