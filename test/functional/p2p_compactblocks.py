...
# TestP2PConn: A peer we use to send messages to xogecoind, and store responses.
...
class CompactBlocksTest(QogecoinTestFramework):
    ...
    # This test actually causes xogecoind to (reasonably!) disconnect us, so do this last.
    ...
    # Test that xogecoind requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for xogecoind to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # xogecoind will not send blocktxn responses for blocks whose height is
