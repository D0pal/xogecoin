Qogecoin Core version 0.14.0 is now available from:
  ...
  <https://xogecoin.org/bin/xogecoin-core-0.14.0/>
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/xogecoin/xogecoin/blob/master/doc/zmq.md) for
...
Commands sent over the JSON-RPC interface and through the `xogecoin-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`xogecoin-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/xogecoin-cli -named help command="help"
    src/xogecoin-cli -named getblockhash height=0
    src/xogecoin-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/xogecoin-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was [disabled and deprecated](https://xogecoin.org/en/alert/2016-11-01-alert-retirement) in Qogecoin Core 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   ...
   optimizations to memory management. See [Pull #8753](https://github.com/xogecoin/xogecoin/pull/8753)
...
- #8722 `bb843ad` xogecoin-cli: More detailed error reporting (laanwj)
...
- #8830 `ef0801b` test: Add option to run xogecoin-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to xogecoin-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to xogecoin-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and xogecoin-util-test.py (jnewbery)
...
- #9691 `fc67cd2` Init ECC context for `test_xogecoin_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change xogecoin address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` xogecoind: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow xogecoin-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` xogecoin-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to xogecoin-tx (jnewbery)
