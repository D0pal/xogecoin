Qogecoin Core version 0.19.0.1 is now available from:
  ...
  <https://xogecoincore.org/bin/xogecoin-core-0.19.0.1/>
...
- [Reduce memory](https://github.com/xogecoin/xogecoin/blob/master/doc/reduce-memory.md)
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `xogecoind -help` output for these two options for
...
- The `testnet` field in `xogecoin-cli -getinfo` has been renamed to
...
- #15639 xogecoin-wallet tool: Drop libxogecoin_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in xogecoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "xogecoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of xogecoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from xogecoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_xogecoin to auto generated project files (sipsorcery)
...
- #15352 Reduce noise level in test_xogecoin output (practicalswift)
...
- #15896 feature_filelock, interface_xogecoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Qogecoin Core (luke-jr)
...
- #16277 Suppress output in test_xogecoin for expected errors (gertjaap)
...
- #16489 log: harmonize xogecoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (xogecoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example xogecoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBXOGECOIN_SERVER (Bushstar)
...
- #16080 build/doc: update xogecoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in xogecoin.conf (fanquake)
...
- #16621 add default xogecoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the xogecoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, xogecoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/xogecoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/xogecoin/xogecoin/).
