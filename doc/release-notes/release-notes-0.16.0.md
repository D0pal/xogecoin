Qogecoin Core version 0.16.0 is now available from:
  ...
  <https://xogecoincore.org/bin/xogecoin-core-0.16.0/>
...
xogecoin data directory. The behavior is now:
...
- xogecoin-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- xogecoin-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 18332.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest xogecoin.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.xogecoin.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for xogecoin-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to xogecoin-cli to allow RPC password to be read from standard input (jharvell)
...
- #11621 `1f7695b` [build] Add temp_xogecoin_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` xogecoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.xogecoin doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for xogecoin-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in xogecoin-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop xogecoind (OmeGak)
...
- #10529 `331352f` Improve xogecoind systemd service file (Flowdalic)
