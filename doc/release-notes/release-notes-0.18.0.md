Qogecoin Core version 0.18.0 is now available from:
  ...
  <https://xogecoincore.org/bin/xogecoin-core-0.18.0/>
...
`/Applications/Qogecoin-Qt` (on Mac) or `xogecoind`/`xogecoin-qt` (on
Linux).
...
`contrib/init/xogecoind.service`, it has been changed to use
`/var/lib/xogecoind` as the data directory instead of
`~xogecoin/.xogecoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/xogecoind` will
exist has enough space (check using `df -h /var/lib/xogecoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/xogecoin/xogecoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://xogecoincore.org/en/contact/) the Qogecoin Core
...
The systemd init file (`contrib/init/xogecoind.service`) has been changed
to use `/var/lib/xogecoind` as the data directory instead of
`~xogecoin/.xogecoin`. This change makes Qogecoin Core more consistent with
...
`datadir` option in `/etc/xogecoin/xogecoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/xogecoin/xogecoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/xogecoin/xogecoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/xogecoin/xogecoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/xogecoin/xogecoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/xogecoin/xogecoin/blob/master/doc/xogecoin-conf.md)
  about the `xogecoin.conf` file describes how to use it to configure
  Qogecoin Core.

- A new document introduces Qogecoin Core's BIP174 [Partially-Signed
  Qogecoin Transactions
  (PSBT)](https://github.com/xogecoin/xogecoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/xogecoin/xogecoin/blob/master/doc/descriptors.md)
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/xogecoin/xogecoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/xogecoin/xogecoin/blob/master/doc/descriptors.md).
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/xogecoin/xogecoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/xogecoin/xogecoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart xogecoind with the `-deprecatedrpc=generate`
...
- A new `xogecoin-wallet` tool is now distributed alongside Qogecoin
...
- A sub-project of Qogecoin Core now provides Hardware Wallet Interaction
  ...
  page](https://github.com/xogecoin-core/HWI#readme) for details.
...
- #12490 Remove deprecated wallet rpc features from `xogecoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libxogecoin_server.a` (jnewbery)
...
- #13926 [Tools] xogecoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like xogecoind (MarcoFalke)
...
- #14521 qt, docs: Fix `xogecoin-qt -version` output formatting (hebasto)
...
- #14204 Move `interfaces/*` to `libxogecoin_server` (laanwj)
...
- #15154 Configure: xogecoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/xogecoin-wallet (Sjors)
...
- #15110 build\_msvc: Fix the build problem in `libxogecoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in xogecoin-util-test.py (isghe)
...
- #12246 Bugfix: Only run xogecoin-tx tests when xogecoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_xogecoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_xogecoin` (MarcoFalke)
...
- #14186 xogecoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_xogecoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update xogecoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/xogecoin-conf.md (hebasto)
...
- #15354 Add missing `xogecoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct xogecoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support xogecoin folder on OSX (JimmyMow)
