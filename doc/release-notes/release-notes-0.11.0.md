Qogecoin Core version 0.11.0 is now available from:
  ...
  <https://xogecoin.org/bin/xogecoin-core-0.11.0/>
...
For example, add the following to `xogecoin.conf`:
...
related to the blockchain in the xogecoin system: the raw blocks as received over 
...
For an example script see [xogecoin-submittx](https://github.com/laanwj/xogecoin-submittx).
...
- #5334 `cf87536` libxogecoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new xogecoin-detached-sigs git repo for OSX signatures
...
- #5753 `0f44672` Add xogecoin logo to about screen
...
- #5809 `46bfbe7` Add xogecoin-cli man page
...
- #5849 `41259ca` contrib/init/xogecoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6098 `7708fcd` Update Windows resource files (and add one for xogecoin-tx)
