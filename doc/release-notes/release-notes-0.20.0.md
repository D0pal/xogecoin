...
Qogecoin Core version 0.20.0 is now available from:
  ...
  <https://xogecoincore.org/bin/xogecoin-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `XOGECOIN_GENBUILD_NO_GIT=1 make`.
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/xogecoin/xogecoin/issues/16599),
  [PR #16702](https://github.com/xogecoin/xogecoin/pull/16702), and the
  `xogecoind help` for more information.  This option is experimental and
...
- A "known bug" [announced](https://xogecoincore.org/en/releases/0.18.0/#wallet-gui)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  program such as [HWI](https://github.com/xogecoin-core/HWI) for
...
- Qogecoin Core's automatically-generated source code documentation is
  now available at https://doxygen.xogecoincore.org.  (#17596)
...
- The `xogecoin-cli` utility used with the `-getinfo` parameter now
...
- The `xogecoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `xogecoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_xogecoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBXOGECOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_xogecoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBXOGECOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for xogecoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate xogecoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest xogecoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_xogecoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (xogecoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone xogecoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #17119 Fix broken xogecoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from xogecoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.xogecoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to xogecoin-wallet (hel-o)
...
- #16115 On xogecoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from xogecoin-cli -getinfo (malevolent)
