Qogecoin Core version 0.12.0 is now available from:
  ...
  <https://xogecoin.org/bin/xogecoin-core-0.12.0/>
...
[libsecp256k1](https://github.com/xogecoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/xogecoin/bips/blob/master/bip-0130.mediawiki)
...
(https://github.com/xogecoin/bips/blob/master/bip-0125.mediawiki).
...
This allows running xogecoind without having to do any manual configuration.
...
`xogecoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/xogecoin/xogecoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/xogecoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/xogecoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /xogecoinrpc>
        ...
        # optional bypass xogecoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< xogecoinrpc:<password>
    </Location>
...
Restarting xogecoind will no longer clear out the list of banned peers; instead
...
- #6269 `d4565b6` gitian: Use the new xogecoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include xogecoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix xogecoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of xogecoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override XOGECOIND and XOGECOINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [xogecoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add XOGECOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libxogecoinconsensus (Braydon Fuller)
