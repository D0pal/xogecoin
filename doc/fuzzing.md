...
```sh
$ git clone https://github.com/xogecoin/xogecoin
$ cd xogecoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/xogecoin/xogecoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/xogecoin_report.pdf) includes an external overview of the status of Qogecoin Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/xogecoin/xogecoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/xogecoin/xogecoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/xogecoin/xogecoin/tree/master/src/test/fuzz).
...
It is possible to specify `xogecoind` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`xogecoin-core/qa-assets`](https://github.com/xogecoin-core/qa-assets) repo.

To fuzz `process_message` using the [`xogecoin-core/qa-assets`](https://github.com/xogecoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/xogecoin-core/qa-assets
...
Fuzzing on a harness compiled with `--with-sanitizers=address,fuzzer,undefined` is good for finding bugs. However, the very slow execution even under libFuzzer will limit the ability to find new coverage. A good approach is to perform occasional long runs without the additional bug-detectors (configure `--with-sanitizers=fuzzer`) and then merge new inputs into a corpus as described in the qa-assets repo (https://github.com/xogecoin-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).  Patience is useful; even with improved throughput, libFuzzer may need days and 10s of millions of executions to reach deep/hard targets.
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`xogecoin-core/qa-assets`](https://github.com/xogecoin-core/qa-assets) repo.

Every single pull request submitted against the Qogecoin Core repo is automatically tested against all inputs in the [`xogecoin-core/qa-assets`](https://github.com/xogecoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Qogecoin Core more robust.
...
with certain assembly code from Qogecoin Core's code. See [developer notes on sanitizers](https://github.com/xogecoin/xogecoin/blob/master/doc/developer-notes.md#sanitizers)
...
Core without having to write any custom fuzzing harness. The `xogecoind` server
...
```sh
$ mkdir xogecoin-honggfuzz-p2p/
$ cd xogecoin-honggfuzz-p2p/
$ git clone https://github.com/xogecoin/xogecoin
$ cd xogecoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/xogecoind.cpp b/src/xogecoind.cpp
...
See [this proposed patch](https://github.com/xogecoin/xogecoin/pull/22472) for more information.
...
Qogecoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/xogecoin-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=xogecoin-core).
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_xogecoin-core/latest).
