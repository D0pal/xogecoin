...
- ClientModel is used by the main application `xogecoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a xogecoin:... link from a browser).
...
* `xogecoinstrings.cpp`: automatically generated
* `xogecoinunits.(h/cpp)`: Qoge / mQoge / etc. handling
* `callback.h`
...
**Note:** Do not change `local/xogecoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `xogecoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `xogecoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `xogecoin-qt`
