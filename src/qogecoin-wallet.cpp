...
static bool WalletAppInit(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(args) || args.IsArgSet("-version")) {
        std::string strUsage = strprintf("%s xogecoin-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n";

        if (args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                        "xogecoin-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n"
                        "By default xogecoin-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n"
                        "To change the target wallet, use the -datadir, -wallet and -regtest/-signet/-testnet arguments.\n\n"
                        "Usage:\n"
                        "  xogecoin-wallet [options] <command>\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
...
int main(int argc, char* argv[])
{
    ...
    if (!command) {
        tfm::format(std::cerr, "No method provided. Run `xogecoin-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
