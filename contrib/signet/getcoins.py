...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing xogecoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='xogecoin-cli', help='xogecoin-cli command to use')
...
parser.add_argument('xogecoin_cli_args', nargs='*', help='Arguments to pass on to xogecoin-cli (default: -signet)')

args = parser.parse_args()

if args.xogecoin_cli_args == []:
    args.xogecoin_cli_args = ['-signet']


def xogecoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.xogecoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = xogecoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = xogecoin_cli(['getnewaddress', 'faucet', 'bech32'])
